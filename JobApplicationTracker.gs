...
const CONFIG = {
  ...
  LOOKBACK_DAYS: 7,
  LABEL: 'Job Search',
  BASE_QUERY: '("thank you for applying" OR "application received" OR subject:applied OR ' +
...
const HEADERS = ['Date', 'Company', 'Role', 'Status', 'Subject', 'From', 'ThreadId', 'MessageId', 'Link'];
const SUMMARY_HEADERS = ['Date', 'Applications', 'Rejections', 'Last Updated'];

function updateJobTracker() {
  console.log(`[JobTracker] Run at ${new Date().toISOString()} TZ=${Session.getScriptTimeZone()}`);
  ...
  const threads = fetchAllThreads_(query, 500);
  const rowsToAppend = [];
  ...
  if (rowsToAppend.length) {
    ...
    appendProcessedIds_(procSheet, rowsToAppend.map(r => r[7]));
  }
...
function onOpen() {
  SpreadsheetApp.getUi().createMenu('Job Tracker')
    ...
    .addItem('Enrich Existing Rows', 'enrichExistingRows')
...
function ensureDailyTrigger_(hour) {
  const fn = 'updateJobTracker';
  const triggers = ScriptApp.getProjectTriggers()
    .filter(t => t.getHandlerFunction() === fn && t.getEventType() === ScriptApp.EventType.CLOCK);

  if (triggers.length > 1) triggers.forEach(t => ScriptApp.deleteTrigger(t));
  if (triggers.length === 0) {
    ScriptApp.newTrigger(fn).timeBased().everyDays(1).atHour(hour).create();
    Logger.log(`Created daily trigger for ${fn} at hour ${hour}`);
  }
}
...
function buildQuery_(_sinceDate) {
...
function cleanCompany_(name) {
  return titleCase_(
    (name || '')
      .replace(/^[\s"'[\]()]+|[\s"'[\]()]+$/g, '')
      .replace(/\s+(careers|recruiting|talent(?:\s+acquisition)?|notifications?|support|team)$/i, '')
      .trim()
  );
}
...
function updateDailySummary() {
  ...
  data.forEach(row => {
    ...
    if (!counts[key]) counts[key] = { apps: 0, rejects: 0 };
    if (/^Applied\/Auto-ack$/i.test(status)) counts[key].apps++;
    else if (/^Rejected$/i.test(status)) counts[key].rejects++;
  });

  const keys = Object.keys(counts).sort();
  const ts = Utilities.formatDate(new Date(), Session.getScriptTimeZone(), 'yyyy-MM-dd HH:mm');

  const rows = keys.map(k => [dateFromKey_(k), counts[k].apps, counts[k].rejects, ts]);

  if (rows.length) {
    sumSh.getRange(2, 1, rows.length, SUMMARY_HEADERS.length).setValues(rows);
    sumSh.getRange(2, 1, rows.length, 1).setNumberFormat('mmm dd yyyy');
  }
}
